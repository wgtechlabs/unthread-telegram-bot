...
export class GlobalTemplateManager {
  ...
  async initializeDefaultTemplates(
    ...
    try {
      ...
      if (
        currentConfig.version > 1 ||
        Object.keys(currentConfig.templates).length ===
          Object.keys(DEFAULT_GLOBAL_TEMPLATES.templates).length
      ) {
      ...
      LogEngine.error('Failed to initialize default templates', {
        error,
        groupChatId,
      })
  ...
  private validateTemplate(content: string): {
    ...
    const dangerousPatterns = [
      ...
      {
        pattern: /document\./i,
        message: 'Document object access is not allowed',
      },
