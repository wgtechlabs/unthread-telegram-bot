  ...
  return async (ctx: BotContext) => {
    try {
      ...
      try {
        await safeReply(
          ctx,
          `Sorry, there was an error processing the ${commandName} command.`
        );
      } catch (replyError) {
...
bot.command(
  'version',
  commandMiddleware,
  wrapCommandHandler(versionCommand, 'version')
);
bot.command(
  'about',
  commandMiddleware,
  wrapCommandHandler(aboutCommand, 'about')
);
bot.command(
  'support',
  commandMiddleware,
  wrapCommandHandler(supportCommand, 'support')
);
bot.command(
  'cancel',
  commandMiddleware,
  wrapCommandHandler(cancelCommand, 'cancel')
);
bot.command(
  'reset',
  commandMiddleware,
  wrapCommandHandler(resetCommand, 'reset')
);
 ...
 */
async function cleanupDatabaseOnInitFailure(
  ...
  if (dbInitialized) {
    try {
      ...
      LogEngine.error(
        'Failed to cleanup database during initialization failure',
        {
...
try {
  ...
  const err = error as Error;
  LogEngine.error(
    'Failed to initialize database or storage after all retry attempts',
    {
      error: err.message,
      maxRetries: 5,
    }
...
try {
  ...
  if (process.env.WEBHOOK_REDIS_URL) {
    ...
    if (typeof webhookHandler.handleConversationUpdated === 'function') {
      ...
      LogEngine.warn(
        'Webhook handler does not implement handleConversationUpdated; skipping subscription.'
      );
    }
    ...
    LogEngine.warn(
      'Webhook Redis URL not configured - webhook processing disabled'
    );
...
await bot.telegram.setMyCommands([
  ...
  {
    command: 'support',
    description: 'Create a support ticket (group chats only)',
  },
