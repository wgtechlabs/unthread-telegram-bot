...
export class TelegramWebhookHandler {
  ...
  async handleMessageCreated(event: any): Promise<void> {
    try {
      ...
      LogEngine.info('🔍 Looking up ticket for conversation', {
        conversationId,
      })

      // 2. Look up original ticket message using bots-brain
      const ticketData =
        await this.botsStore.getTicketByConversationId(conversationId)
      if (!ticketData) {
      ...
      try {
        ...
        if (sentMessage) {
          ...
          LogEngine.info(
            '✅🎉 Agent message delivered to Telegram successfully!',
            {
              conversationId,
              chatId: ticketData.chatId,
              replyToMessageId: ticketData.messageId,
          ...
          LogEngine.warn(
            'Message not sent - user may have blocked bot or chat not found',
            {
              conversationId,
              chatId: ticketData.chatId,
              friendlyId: ticketData.friendlyId,
        ...
        try {
          ...
          if (fallbackMessage) {
            ...
            LogEngine.warn(
              'Fallback message also failed - user may have blocked bot',
              {
                conversationId,
                chatId: ticketData.chatId,
              }
  ...
  formatAgentMessage(text: string, friendlyId: string): string {
    ...
    if (cleanText.length > maxLength) {
      truncatedText =
        cleanText.substring(0, maxLength - 50) + '...\n\n_Message truncated_'
    }
  ...
  async handleConversationUpdated(event: any): Promise<void> {
    try {
      ...
      LogEngine.info('🔍 Looking up ticket for status update', {
        conversationId,
        newStatus,
      })
      ...
      try {
        ...
        if (sentMessage) {
          LogEngine.info(
            '✅🎉 Status notification delivered to Telegram successfully!',
            {
              conversationId,
              chatId: ticketData.chatId,
              replyToMessageId: ticketData.messageId,
          ...
          LogEngine.warn(
            'Status notification not sent - user may have blocked bot',
            {
              conversationId,
              chatId: ticketData.chatId,
              friendlyId: ticketData.friendlyId,
        ...
        try {
          ...
          if (fallbackMessage) {
            LogEngine.info(
              'Status notification sent as new message (fallback)',
              {
                conversationId,
                chatId: ticketData.chatId,
                newStatus,
            ...
            LogEngine.warn(
              'Fallback status notification also failed - user may have blocked bot',
              {
                conversationId,
                chatId: ticketData.chatId,
                newStatus,
          ...
          const fallbackErr = fallbackError as Error
          LogEngine.error(
            'Failed to send fallback status notification to Telegram',
            {
              error: fallbackErr.message,
              chatId: ticketData.chatId,
              conversationId,
   ...
   * @returns Formatted message
   */
  async formatAgentMessageWithTemplate(
    text: string,
    ticketData: any,
    eventData: any
  ): Promise<string> {
    try {
      ...
      const formatted = await this.templateManager.renderTemplate(
        'agent_response',
        variables
      )
   ...
   * @returns Formatted message
   */
  async formatStatusUpdateWithTemplate(
    ticketData: any,
    status: string,
    eventData: any
  ): Promise<string> {
    try {
      ...
      LogEngine.warn(
        'Failed to format status update with template, using fallback',
        {
          error: error instanceof Error ? error.message : 'Unknown error',
          ticketId: ticketData.friendlyId,
          status,
  ...
  async cleanupBlockedUser(chatId: number): Promise<void> {
    try {
      ...
      if (customer) {
        ...
        await this.botsStore.storage.delete(
