...
export async function handleMessage(
  ...
  try {
    ...
    LogEngine.debug('Processing message', {
      ...
      userId: ctx.from?.id,
      messageText:
        'text' in ctx.message ? ctx.message.text?.substring(0, 50) : undefined,
      isCommand:
    ...
    if (isPrivateChat(ctx)) {
      LogEngine.debug(
        'Processing as private message (no conversation processor handled it)'
      )
 ...
 */
async function handleTicketConfirmationReply(
  ctx: BotContext,
  ticketInfo: any
): Promise<boolean> {
  try {
 ...
 */
async function validateTicketReply(
  ctx: BotContext,
  ticketInfo: any
): Promise<
  | { isValid: false }
 ...
 */
async function processTicketMessage(
  ticketInfo: any,
  telegramUserId: number,
  username: string | undefined,
  message: string
 ...
 */
async function handleAgentMessageReply(
  ctx: BotContext,
  agentMessageInfo: any
): Promise<boolean> {
  try {
    if (!ctx.from || !ctx.message || !('text' in ctx.message)) {
      return false
