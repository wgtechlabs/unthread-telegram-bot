...
export class SupportConversationProcessor implements IConversationProcessor {
  ...
  private async handleSummaryInput(
    ctx: BotContext,
    summary: string,
    userState: any
  ): Promise<boolean> {
  ...
  private async handleEmailInput(
    ...
    const emailValidation = await import('../utils/validation.js').then(
      (module) => module.validateEmail(email.trim())
    )
  ...
  private async createTicket(
    ...
    try {
      ...
      const customer = await unthreadService.getOrCreateCustomer(
        chatTitle,
        chatId
      )

      // Get user data with email
      let userData = await unthreadService.getOrCreateUser(
        userId,
        ctx.from?.username
      )
      ...
      if (
        userState.email &&
        (!userData.email || userData.email !== userState.email)
      ) {
      ...
      logError(error, 'SupportConversationProcessor.createTicket', {
        userId,
        chatId,
      })
...
export class DmSetupInputProcessor implements IConversationProcessor {
  async canHandle(ctx: BotContext): Promise<boolean> {
    ...
    try {
      ...
      const activeSessions =
        await BotsStore.getActiveDmSetupSessionByAdmin(userId)
      ...
      if (sessionExpiry <= now) {
        ...
        const timeDiffMinutes =
          (now.getTime() - sessionExpiry.getTime()) / (1000 * 60)
      ...
      const canHandle =
        activeSessions.currentStep === 'awaiting_custom_name' ||
  ...
  async process(ctx: BotContext): Promise<boolean> {
    ...
    try {
      ...
      if (sessionExpiry.getTime() - now.getTime() < 5 * 60 * 1000) {
      ...
      if (
        session.currentStep !== 'awaiting_custom_name' &&
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {
                  text: '❌ Cancel Setup',
                  callback_data: `setup_cancel_unknown`,
                },
              ],
            ],
          },
  ...
  private async handleCustomerIdInput(
    ctx: BotContext,
    session: any,
    customerId: string
  ): Promise<boolean> {
    try {
      ...
      try {
        await this.createGroupConfigForCustomer(
          session,
          trimmedCustomerId,
          customerName
        )

        const successMessage = this.generateSetupSuccessMessage(
          customerName,
          trimmedCustomerId,
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {
                  text: '✅ Finish Setup',
                  callback_data: `setup_use_defaults_${session.sessionId}`,
                },
                {
                  text: '🎨 Customize Templates',
                  callback_data: `setup_customize_templates_${session.sessionId}`,
                },
              ],
            ],
          },
        ...
        logError(
          setupError,
          'DmSetupInputProcessor.completeExistingCustomerSetup',
          {
          ...
          '❌ **Setup Failed**\n\nFailed to complete the customer setup. This might be due to:\n• Database connection issues\n• Invalid session state\n• System configuration problems\n\nWhat would you like to do?',
          {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: '🔄 Retry Customer ID',
                    callback_data: `setup_existing_customer_${session.sessionId}`,
                  },
                  {
                    text: '✏️ Use Different Name',
                    callback_data: `setup_custom_name_${session.sessionId}`,
                  },
                ],
                [
                  {
                    text: '⬅️ Back to Options',
                    callback_data: `setup_back_to_customer_selection_${session.sessionId}`,
                  },
                ],
                [
                  {
                    text: '❌ Cancel Setup',
                    callback_data: `setup_cancel_${session.sessionId}`,
                  },
                ],
              ],
            },
  ...
  private async handleCustomNameInput(
    ctx: BotContext,
    session: any,
    inputText: string
  ): Promise<boolean> {
    try {
      ...
      if (!validation.isValid) {
        ...
        const { SetupCallbackProcessor } = await import(
          './CallbackProcessors.js'
        )
        const shortBackId = SetupCallbackProcessor.generateShortCallbackId(
          session.sessionId
        )
        const shortCancelId = SetupCallbackProcessor.generateShortCallbackId(
          session.sessionId
        )
      ...
      await ctx.reply('✅ Processing your custom customer name...', {
        parse_mode: 'Markdown',
      })
      ...
      await callbackProcessor.completeCustomerSetup(
        ctx,
        session.sessionId,
        customerName,
        session
      ...
      const shortCustomId = SetupCallbackProcessor.generateShortCallbackId(
        session.sessionId
      )
      const shortExistingId = SetupCallbackProcessor.generateShortCallbackId(
        session.sessionId
      )
      const shortBackId = SetupCallbackProcessor.generateShortCallbackId(
        session.sessionId
   ...
   */
  private async handleTemplateContentInput(
    ctx: BotContext,
    session: any,
    ...
    try {
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: '❌ Cancel Edit',
                    callback_data: `template_cancel_edit_${session.stepData?.editingTemplateType}_${session.sessionId}`,
                  },
                ],
              ],
            },
            ...
            reply_markup: {
              inline_keyboard: [
                [
                  {
                    text: '❌ Cancel Edit',
                    callback_data: `template_cancel_edit_${templateType}_${session.sessionId}`,
                  },
                ],
              ],
            },
      ...
      await callbackProcessor.showTemplateCustomization(
        ctx,
        session.sessionId,
        session
      )
   ...
   */
  private async validateCustomerId(
    ctx: BotContext,
    customerId: string,
    session: any
  ): Promise<{
    ...
    const validationMsg = await ctx.reply(
      '🔍 Validating customer ID with Unthread...',
      { parse_mode: 'Markdown' }
    )
    ...
    try {
      ...
      const customerName =
        validationResult.customer?.name ||
        `Customer ${trimmedCustomerId.substring(0, 8)}...`
      ...
      const { BotsStore } = await import('../../sdk/bots-brain/index.js')
      const currentSession = await BotsStore.getDmSetupSession(
        session.sessionId
      )
      ...
      throw new Error(
        `Customer validation failed: ${(validationError as Error).message}`
      )
    }
  }
   ...
   */
  private async createGroupConfigForCustomer(
    session: any,
    customerId: string,
    customerName: string
  ): Promise<void> {
    ...
    const newExpiryFromCurrent = new Date(
      currentExpiry.getTime() + 30 * 60 * 1000
    )
   ...
   */
  private generateSetupSuccessMessage(
    customerName: string,
    customerId: string,
    groupName: string
  ): string {
