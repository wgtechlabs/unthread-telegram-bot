...
export const processSupportConversation = async (
  ...
  try {
    ...
    if (
      userState &&
      userState.initiatedBy &&
      userState.initiatedBy !== telegramUserId
    ) {
      // This message is from a different user, ignore it silently
      LogEngine.debug(
        'Ignoring message from different user during support flow',
        {
    ...
    if (
      userState &&
      userState.initiatedInChat &&
      userState.initiatedInChat !== ctx.chat?.id
    ) {
      // This message is from a different chat, ignore it
      LogEngine.debug(
        'Ignoring message from different chat during support flow',
        {
    ...
    if (ctx.callbackQuery) {
      if ('data' in ctx.callbackQuery) {
        ...
        if (callbackData === 'skip_email') {
          if (
            (userState.currentField || userState.field) ===
            SupportFieldEnum.EMAIL
          ) {
 ...
 */
async function handleEmailField(
  ctx: BotContext,
  userState: any,
  messageText: string
): Promise<void> {
