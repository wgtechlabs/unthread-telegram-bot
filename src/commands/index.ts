...
async function handleEmailField(ctx: BotContext, userState: any, messageText: string): Promise<void> {
    try {
        ...
        try {
            ...
            // Step 3: Update user's email if they provided one during this ticket creation
            if (userState.ticket.email && userState.ticket.email !== userData.email) {
                // Check if this is a real email or auto-generated
                const isAutoGenerated = userState.ticket.email.endsWith('@telegram.user');
                
                // Always update the user's stored email, whether real or auto-generated
                // This ensures consistency between what was used for the ticket and what's stored
                const { updateUserEmail } = await import('../utils/emailManager.js');
                const updateResult = await updateUserEmail(telegramUserId, userState.ticket.email, isAutoGenerated);
                
                if (updateResult.success) {
                    LogEngine.info('Updated user email from ticket creation', {
                        telegramUserId,
                        previousEmail: userData.email,
                        newEmail: userState.ticket.email,
                        isAutoGenerated
                    });
                } else {
                    LogEngine.warn('Failed to update user email during ticket creation', {
                        telegramUserId,
            ...
            // Step 4: Create a ticket with the customer ID and user data
            const ticketResponse = await unthreadService.createTicket({
                ...
                onBehalfOf: {
                    name: userData.name,
                    email: userState.ticket.email // Use the email from the current ticket flow
                }
            });
            
            // Step 5: Generate success message with ticket ID
 ...
 * provided during ticket creation or an auto-generated dummy email.
 * Based on the advanced implementation from fix/support-ticket-creation branch.
 */
const viewEmailCommand = async (ctx: BotContext): Promise<void> => {
    try {
        const telegramUserId = ctx.from?.id;
        if (!telegramUserId) {
            await safeReply(ctx, "❌ Unable to process request. Please try again.");
            return;
        }

        const { getUserEmailPreferences, generateDummyEmail } = await import('../utils/emailManager.js');
        const { formatEmailForDisplay } = await import('../utils/markdownEscape.js');
        
        const emailPrefs = await getUserEmailPreferences(telegramUserId);
        
        if (!emailPrefs) {
            await showNoEmailMessage(ctx);
            return;
        }

        await showEmailSettings(ctx, emailPrefs, formatEmailForDisplay);
...
/**
 * Shows the no email message with setup options
 */
async function showNoEmailMessage(ctx: BotContext): Promise<void> {
    const message = 
        "📧 **Email Settings**\n\n" +
...
/**
 * Shows the user's email settings with appropriate actions
 */
async function showEmailSettings(ctx: BotContext, emailPrefs: any, formatEmailForDisplay: Function): Promise<void> {
    ...
    if (emailPrefs.isDummy) {
        ...
        message += "• Create a new support ticket to update your email\n";
