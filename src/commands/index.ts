    ...
    try {
        ...
        try {
            ...
            if (userState.ticket.email && userState.ticket.email !== userData.email) {
                ...
                await unthreadService.updateUserEmail(telegramUserId, userState.ticket.email);
...
const viewEmailCommand = async (ctx: BotContext): Promise<void> => {
    try {
        ...
        const userData = await BotsStore.getUserByTelegramId(telegramUserId);
        
        let userEmail: string;
        let emailSource: string;

        if (userData && userData.unthreadEmail) {
            // User has a stored email
            userEmail = userData.unthreadEmail;
            emailSource = userData.unthreadEmail.endsWith('@telegram.user') ? 'auto-generated' : 'provided';
        } else {
            // Generate fallback email in the same format as the support flow
            const username = ctx.from?.username || 'user';
            userEmail = `${username}_${telegramUserId}@telegram.user`;
            emailSource = 'auto-generated (fallback)';
        }

        const emailMessage = `📧 **Your Email Address**\n\n` +
            `**Email:** \`${userEmail}\`\n` +
            `**Source:** ${emailSource}\n\n` +
            `${emailSource.includes('auto-generated') 
                ? '💡 This email was automatically generated. You can provide a real email when creating support tickets.' 
                : '✅ This email was provided during ticket creation.'}`;
