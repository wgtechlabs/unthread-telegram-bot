  ...
  async connect(): Promise<void> {
    try {
      ...
      if (this.redisConfig.url) {
        ...
        LogEngine.info(
          'Redis URL not provided, using Memory + PostgreSQL only'
        );
      }
      ...
      this.connected = true;
      LogEngine.info(
        'UnifiedStorage initialized with multi-layer architecture'
      );
    } catch (error) {
  ...
  private async setInPostgres(
    ...
    try {
      const expiresAt = new Date(
        Date.now() + (ttl || this.redisConfig.ttl) * 1000
      );
      await this.db!.query(
        `
      ...
      // Table might not exist, that's okay for now
      LogEngine.debug(
        'storage_cache table not found, using Redis + Memory only'
      );
    }
  }
