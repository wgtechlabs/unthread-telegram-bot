...
export class BotsStore implements IBotsStore {
  ...
  static getInstance(): BotsStore {
    if (!BotsStore.instance) {
      throw new Error(
        'BotsStore not initialized. Call BotsStore.initialize() first.'
      )
    }
    return BotsStore.instance
  }
  ...
  static async getCustomerByChatId(
    chatId: number
  ): Promise<CustomerData | null> {
    ...
    try {
      ...
      const promises = [
        ...
        this.storage.set(
          ...
          JSON.stringify(enrichedTicketData)
        ),

        // Lookup by Unthread conversation ID
        this.storage.set(
          `ticket:unthread:${conversationId}`,
          JSON.stringify(enrichedTicketData)
        ),
  ...
  async getTicketsForChat(chatId: number): Promise<TicketData[]> {
    ...
    const chatTickets: TicketInfo[] = data
      ? typeof data === 'string'
        ? JSON.parse(data)
        : data
      : []
  ...
  async storeCustomer(customerData: CustomerData): Promise<boolean> {
    ...
    try {
      await Promise.all([
        ...
        this.storage.set(
          `customer:id:${unthreadCustomerId}`,
          JSON.stringify(enrichedCustomerData)
        ),

        // Lookup by chat ID for quick access
        this.storage.set(
          `customer:telegram:${telegramChatId}`,
          JSON.stringify(enrichedCustomerData)
        ),
      ])

      LogEngine.info(
        `Customer stored: ${name || company} (${unthreadCustomerId})`
      )
      return true
    } catch (error) {
      const err = error as Error
  ...
  async storeUser(userData: UserData): Promise<boolean> {
    ...
    try {
      ...
      await this.storage.set(
        ...
        JSON.stringify(enrichedUserData)
      )

      LogEngine.info(
        `User stored: ${firstName} ${lastName || ''} (${telegramUserId})`
      )
      return true
    } catch (error) {
      const err = error as Error
  ...
  async getCustomerByUnthreadId(
    ...
    const fallbackData = await this.storage.get(
      `customer:unthread:${unthreadCustomerId}`
    )
    return fallbackData
  ...
  async getOrCreateCustomer(
    ...
    try {
      ...
      if (existingCustomer) {
        LogEngine.info(
          `Found existing customer for chat ${chatId}: ${existingCustomer.unthreadCustomerId}`
        )
   ...
   */
  private async addToChatTickets(
    chatId: number,
    messageId: number,
    conversationId: string
  ): Promise<void> {
    ...
    const existingTickets: TicketInfo[] = data
      ? typeof data === 'string'
        ? JSON.parse(data)
        : data
      : []
   ...
   */
  private async removeFromChatTickets(
    chatId: number,
    conversationId: string
  ): Promise<void> {
    const key = `chat:tickets:${chatId}`
  ...
  async storeAgentMessage(
    ...
    try {
      ...
      await this.storage.set(
        `agent_message:telegram:${messageId}`,
        JSON.stringify(enrichedData)
      )

      LogEngine.info(
        `Agent message stored: ${messageId} for conversation ${conversationId}`
      )
      return true
    } catch (error) {
      const err = error as Error
   ...
   */
  static async storeAgentMessage(
    agentMessageData: AgentMessageData
  ): Promise<boolean> {
    return BotsStore.getInstance().storeAgentMessage(agentMessageData)
  }

  static async getAgentMessageByTelegramId(
    messageId: number
  ...
  async updateGroupConfig(
    ...
    try {
      ...
      if (!existingConfig) {
        LogEngine.warn('Cannot update non-existent group configuration', {
          chatId,
        })
        return false
      }
      ...
      const safeUpdates = BotsStore.excludeImmutableFields(updates, [
        'chatId',
        'customerId',
      ])
  ...
  async storeGroupConfigs(configs: GroupConfig[]): Promise<boolean> {
    try {
      const results = await Promise.all(
        configs.map((config) => this.storeGroupConfig(config))
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to store group configurations', {
  ...
  async getGroupConfigs(chatIds: number[]): Promise<(GroupConfig | null)[]> {
    try {
      return await Promise.all(
        chatIds.map((chatId) => this.getGroupConfig(chatId))
      )
    } catch (error) {
      LogEngine.error('Failed to get group configurations', {
        error: (error as Error).message,
      })
  ...
  async updateGroupConfigs(
    updates: { chatId: number; updates: Partial<GroupConfig> }[]
  ): Promise<boolean> {
    try {
      const results = await Promise.all(
        updates.map((update) =>
          this.updateGroupConfig(update.chatId, update.updates)
        )
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to update group configurations', {
        error: (error as Error).message,
      })
      return false
    }
  ...
  async deleteGroupConfigs(chatIds: number[]): Promise<boolean> {
    try {
      const results = await Promise.all(
        chatIds.map((chatId) => this.deleteGroupConfig(chatId))
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to delete group configurations', {
        error: (error as Error).message,
      })
      return false
    }
  ...
  async storeSetupStates(states: SetupState[]): Promise<boolean> {
    try {
      const results = await Promise.all(
        states.map((state) => this.storeSetupState(state))
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to store setup states', {
        error: (error as Error).message,
      })
      return false
    }
  ...
  async getSetupStates(chatIds: number[]): Promise<(SetupState | null)[]> {
    try {
      return await Promise.all(
        chatIds.map((chatId) => this.getSetupState(chatId))
      )
    } catch (error) {
      LogEngine.error('Failed to get setup states', {
        error: (error as Error).message,
      })
      return chatIds.map(() => null)
    }
  ...
  async updateSetupStates(
    updates: { chatId: number; updates: Partial<SetupState> }[]
  ): Promise<boolean> {
    try {
      const results = await Promise.all(
        updates.map((update) =>
          this.updateSetupState(update.chatId, update.updates)
        )
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to update setup states', {
        error: (error as Error).message,
      })
      return false
    }
  ...
  async clearSetupStates(chatIds: number[]): Promise<boolean> {
    try {
      const results = await Promise.all(
        chatIds.map((chatId) => this.clearSetupState(chatId))
      )
      return results.every((result) => result === true)
    } catch (error) {
      LogEngine.error('Failed to clear setup states', {
        error: (error as Error).message,
      })
      return false
    }
  ...
  async storeAdminProfile(adminData: AdminProfile): Promise<boolean> {
    try {
      ...
      await this.storage.set(
        `admin:profile:${adminData.telegramUserId}`,
        JSON.stringify(enrichedData)
      )
  ...
  async storeSetupSession(sessionData: SetupSession): Promise<boolean> {
    try {
      ...
      // Store by session ID
      await this.storage.set(
        `session:setup:${sessionData.sessionId}`,
        JSON.stringify(enrichedData),
        600
      ) // 10 minutes TTL
      ...
      await this.storage.set(
        `session:admin:${sessionData.initiatingAdminId}`,
      ...
      if (sessionData.groupChatId) {
        await this.storage.set(
          `session:group:${sessionData.groupChatId}`,
          sessionData.sessionId,
          600
        )
      }
   ...
   */
  async getActiveSetupSessionByAdmin(
    adminId: number
   ...
   */
  async getActiveSetupSessionByGroup(
    groupChatId: number
  ): Promise<SetupSession | null> {
  ...
  async storeDmSetupSession(sessionData: DmSetupSession): Promise<boolean> {
    try {
      ...
      if (ttlSeconds <= 300) {
        ...
        LogEngine.warn(
          'Session expiry was too short, extending automatically',
          {
            sessionId: sessionData.sessionId,
   ...
   */
  async getActiveDmSetupSessionByAdmin(
    adminId: number
  ): Promise<DmSetupSession | null> {
    try {
   ...
   */
  async updateDmSetupSession(
    sessionId: string,
    updates: Partial<DmSetupSession>
  ): Promise<boolean> {
    try {
  ...
  static async updateGroupConfig(
    chatId: number,
    updates: Partial<GroupConfig>
  ): Promise<boolean> {
  ...
  private static excludeImmutableFields<T extends Record<string, any>>(
    ...
    for (const field of immutableFields) {
      if (field in safeUpdates) {
        ...
        LogEngine.warn(
          `Attempted to update immutable field '${String(field)}' - ignoring`,
          {
            field: String(field),
  ...
  private static validateImmutableFields<T extends Record<string, any>>(
    ...
    for (const { field, expectedValue } of fieldValidations) {
      if (field in updates && updates[field] !== expectedValue) {
        LogEngine.error(
          `Immutable field validation failed for '${String(field)}'`,
          {
            field: String(field),
            expectedValue,
            attemptedValue: updates[field],
  ...
  private async getArrayFromStorage(key: string): Promise<any[]> {
    ...
    if (typeof data === 'string') {
      try {
        ...
        LogEngine.warn(
          'Failed to parse JSON string from storage, returning empty array',
          {
            key,
