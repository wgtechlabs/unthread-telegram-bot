...
const ENV_VAR_HELP: Record<string, string> = {
  TELEGRAM_BOT_TOKEN:
    'Message @BotFather on Telegram, create a new bot with /newbot',
  UNTHREAD_API_KEY:
    'Login to Unthread dashboard → Settings → API Keys → Generate new key',
  UNTHREAD_SLACK_CHANNEL_ID:
    'Unthread dashboard → Settings → Integrations → Slack → Channel ID',
  UNTHREAD_WEBHOOK_SECRET:
    'Unthread dashboard → Settings → Webhooks → Create webhook → Copy secret',
  ADMIN_USERS:
    'Message @userinfobot on Telegram to get your user ID (comma-separated list)',
  PLATFORM_REDIS_URL: 'Redis connection string for bot state management',
  WEBHOOK_REDIS_URL:
    'Redis connection string for webhook event processing (agent responses)',
...
export function validateEnvironment(): void {
  ...
  if (missingVars.length > 0) {
    ...
    missingVars.forEach((varName) => {
      const help =
        ENV_VAR_HELP[varName] || 'See documentation for setup instructions'
...
function validateRequiredTokens(): void {
  const tokens = [
    ...
    {
      name: 'UNTHREAD_WEBHOOK_SECRET',
      value: process.env.UNTHREAD_WEBHOOK_SECRET,
    },
  ]
...
export function getAdminUsers(): number[] {
  ...
  if (
    placeholderValues.some((placeholder) =>
      adminUsers.toLowerCase().includes(placeholder.toLowerCase())
    )
  ) {
    throw new Error(
...
export function getConfiguredBotUsername(): string | null {
  ...
  if (
    !configuredUsername ||
    placeholderValues.includes(configuredUsername.toLowerCase())
  ) {
  ...
  if (!/^[a-zA-Z0-9_]{5,32}$/.test(configuredUsername)) {
    LogEngine.warn(
      'Invalid BOT_USERNAME format, falling back to API retrieval',
      {
        configuredUsername,
